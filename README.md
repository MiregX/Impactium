...
1. `Prisma.EntityDefaultArgs` используется для типизации метода `Entity.select()`.

   a. В основании используется как `...select()`.
   
   b. В графе используется как `{ entity: options.Entity && ...select() }`.
   
   c. Дополнительные параметры обозначаются как `...(options.something && this.method())`.
   
   d. Опции для селекции типизируются как `EntitySelectOptions`.
